...
• Эстрадиол гель: «Дивигель», «Эстрожель» (иногда без рецепта)
...
*Схема:* 
- Гель — 1–2 мг/день 
- Инъекции — 5–10 мг каждые 7–10 дней
...
async def help_category(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if category == "💼 Юридическая":
        ...
        await update.message.reply_text("Опишите ваш запрос:", reply_markup=ReplyKeyboardRemove())
        return TYPING
...
async def handle_faq(update: Update, context: ContextTypes.DEFAULT_TYPE, mode: str) -> int:
    ...
    if question == "🔙 Назад":
        ...
        await update.message.reply_text("Опишите ваш вопрос:", reply_markup=ReplyKeyboardRemove())
        return TYPING
    else:
